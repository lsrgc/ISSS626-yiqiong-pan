...
With row-standardised weights (style="W"), the spatial lag at area i is just the neighbour-weighted average of the variable.
...
head(hunan)
```
The code chunk next plots both GDPPC and spatial lag GDPPC for comparsion.
...
To compute spatial lag as the sum of neighbours, use binary weights (1 per neighbour). Build a weight list of ones for each neighbour vector, then pass it via `glist` with `style="B"` (no standardisation), so weight returns sums.
...
tmap_arrange(gdppc, lag_gdppc,lag_sum_gdppc, w_avg_gdppc, w_sum_gdppc, asp=1, ncol=2)
