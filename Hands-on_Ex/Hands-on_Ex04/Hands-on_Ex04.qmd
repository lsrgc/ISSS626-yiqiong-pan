...
The code chunk below can display the contents of the matrix, though the outcome is not shown here. We can also call the index of the nb list to display the neigbhours of i-th region, for example with `wm_d62[[1]]`.
...
The code chunk below plots the distance weight matrix. The first map shows connections based on 1st nearest neighbours while the following shows all neighbours based on cut-off boundary of 62km.

```{r}
#| fig-width: 8
#| fig-height: 6
#| echo: false
plot(hunan$geometry, border = "lightgrey", main = "First-nearest-neighbor Distance Neighbours")
...
The code chunk belows plots the formation seperately. 
