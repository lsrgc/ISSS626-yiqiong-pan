...
```
**Meaning of Spatial lag as a sum of neighboring values:**
*It is based on sum (total neighbour count), not an average.*
...
The raw GDPPC map reflects only local wealth, while the spatial lag (average) highlights the influence of neighbouring regions’ wealth. Including the diagonal (lag window average) blends self and neighbours, producing a more conservative measure. In contrast, lag sum and weighted sum emphasise total neighbour wealth and therefore amplify the role of areas with many adjacent regions.



```{r, fig.width= 20, fig.height=20}
tmap_arrange(gdppc, lag_gdppc, w_avg_gdppc,gdppc, lag_sum_gdppc, w_sum_gdppc, ncol = 3, asp = 1)
```
