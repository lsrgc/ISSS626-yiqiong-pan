...
**1.6 Plotting the Geo spatial Data**
...
1.8.1 The chunk below first uses r**ead_csv()** from readr to import listing.csv into R as a tibble data frame, and then applies **list() from base R** to preview the first 10 records, similar to **glimpse() of dplyr.**
...
The code chunk below computes preschool density per km2 ( 1km2 = 1,000,000m2)
...
a. Use hist() of R Graphics to plot a histgram of the distribution of PreSch Density.
```{r}
...
b2. create a scatter plot for PreSch Density and PreSch Count.
```{r}
