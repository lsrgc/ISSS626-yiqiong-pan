...
The G-function `Gest()` Nearest-neighbour CDF. It’s the proportion of points whose nearest neighbour is within distance r (a distribution, not a count). With Monte Carlo simulation `envelope()`, we test against complete spatial randomness (CSR).
...
The black curve is mostly **inside** the envelope under 400m. But it shows the significant clustering beyond 400m.
...
Through multiple tests (GFKL functions) against complete spatial randomness, we can conclude the childcare centres in Choa Chu Kang are highly like clustered below 400m whereas the centres in Tampines are largely clustered since short distances.
