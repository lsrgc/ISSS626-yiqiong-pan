...
a.  The code chunk below calculates the G-function. The plot shows childcare centres are clustered within 300 metres but rather random beyond the scale.
...
b.  the code chunk below test Complete Spatial Randomness.
...
Since the number of simulations is 999, for two-sided test, `nrank` = (`nsim` + 1) \* 0.001 / 2 = 0.5?

`nrank` must be an integer.
...
a.  The code chunk below calculates the G-function.
