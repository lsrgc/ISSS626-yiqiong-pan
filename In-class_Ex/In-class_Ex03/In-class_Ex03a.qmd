...
when converting the sf to ppp, we include a nested `rjitter()` function to push away the overlapped data points.
...
Lcsr_df <- as.data.frame(L_ck.csr)
...
We first convert the CSR object (e.g., L_tm.csr or G_tm.csr) to a table with as.data.frame() and pass the object into a reusable function csr_plot() to generate the plot.
The interactive plot lets us hover to view distances and significance levels.
This makes it easy to see at which distances clustering or inhibition occurs based on colour coding.
.
