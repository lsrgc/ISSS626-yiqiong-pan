...
This report demonstrates how to apply the Spatial and temporal Spatial Point Pattern Analysis through a case study of **new business registration in Singapore** from 1 January to 30 June 2025. To strengthen the analysis, the full year of 2024 is also included as a baseline under Spatio-Temporal SPPA.
...
The business types were selected based on the number of new registrations in the first half of 2025. Wholesale trade and software development consistently ranked among the top sectors in both 2024 and 2025, while food stalls, although not in the top group for 2024 but within the top 10 in 2025, were included for their distinct Singaporean context.
...
is a random set of points, each showing when and where an event happens. Examples include disease cases, animal sightings, or natural disasters such as fires and earthquakes.
...
#### Selecting Busines Type 2024

Similarly we repeat the steps to generate the table for new business registration in 2024 and extract the registration details for the three targeted business types (namely 46900 wholesale trade, 62011 software development and 56140 food stalls).
...
The code chunk below filters out the new entities are not live in 2024. For 2024, 64,815 of 71,515 newly registered entities remain live as at August 2025. That means new business's survival rate is about 91%.

```{r}
#| eval: false
acra_2024l <- acra_2024 %>%
  filter(str_detect(entity_status_description, "Live"))
```

The code chunk below filters out the top 15 business types based on the count of new registrations that remain live. For the study business types, the comparison table shows estimated closure rates of 8% for wholesale trade (46900), 13% for food stalls (56140), and 12% for software development (62011). All three closure rates are higher than those observed in early 2025, which might could be further analysed in a separate study. In this report, however, we focus only on businesses that remain live for the SPPA analysis.
...
top15_24 <- acra_2024 %>%
  filter(str_detect(entity_status_description, "Live")) %>%
  count(primary_ssic_code, sort = TRUE) %>%
  slice_max(n , n =15) %>%   # keep top 20 codes
  mutate(primary_ssic_code = as.character(primary_ssic_code)) %>%
  left_join(ssic_lookup, by = c("primary_ssic_code" = "ssic_code"))
kable(top15_24)
```
...
# Count in full dataset
counts_full <- acra_2024 %>%
  count(primary_ssic_code, name = "count_full") %>%
  filter(primary_ssic_code %in% target_codes)

# Count in live-only dataset
counts_live <- acra_2024l %>%
  count(primary_ssic_code, name = "count_live") %>%
  filter(primary_ssic_code %in% target_codes)
...
kable(counts_compare,caption ="Live vs struck-off for selected business (2024), with estimated closure rates")
```

Next we save and load the business data in 2024.

```{r}
#| eval: false
biz_46900_24 <- acra_2024l %>% filter(primary_ssic_code == "46900") %>% select(c(1, 3, 9, 12:17, 22, 25:27)) #wholesale
biz_62011_24 <- acra_2024l %>% filter(primary_ssic_code == "62011") %>% select(c(1, 3, 9, 12:17, 22, 25:27)) #software
biz_56140_24 <- acra_2024l %>% filter(primary_ssic_code == "56140") %>% select(c(1, 3, 9, 12:17, 22, 25:27)) #food stalls
```

```{r}
#| eval: false
write_rds(biz_46900_24, "data/rds/biz_46900_24.rds")
biz_46900_24 <- read_rds("data/rds/biz_46900_24.rds")
```

```{r}
#| eval: false
write_rds(biz_62011_24, "data/rds/biz_62011_24.rds")
biz_62011_24 <- read_rds("data/rds/biz_62011_24.rds")
```

```{r}
#| eval: false
write_rds(biz_56140_24, "data/rds/biz_56140_24.rds")
biz_56140_24 <- read_rds("data/rds/biz_56140_24.rds")
```
...
geocode_biz <- function(biz_df, out_rds_path) {
  ...
  # Convert to sf
  biz_df <- biz_df %>% filter(!is.na("X"), !is.na("Y"))
...
```{r}
#| eval: false
biz_46900_24_sf <- geocode_biz(biz_46900_24, "data/rds/biz_46900_24.rds")
write_rds(biz_46900_24_sf, "data/rds/biz_46900_24_sf.rds")
```

```{r}
#| eval: false
biz_46900_24_sf<- read_rds("data/rds/biz_46900_24_sf.rds")
```

```{r}
#| eval: false
res <- geocode_biz(biz_62011_24, "data/rds/biz_62011_24.rds")
biz_56140_24_sf <- res$biz_sf
write_rds(biz_62011_24_sf, "data/rds/biz_62011_24_sf.rds")
missing_rows  <- res$bad
missing_rows
```

```{r}
#| eval: false
biz_62011_24_sf <- read_rds("data/rds/biz_62011_24_sf.rds")
```

```{r}
#| eval: false
res <- geocode_biz(biz_56140_24, "data/rds/biz_56140_24.rds")
biz_56140_24_sf <- res$biz_sf
write_rds(biz_56140_24_sf, "data/rds/biz_56140_24_sf.rds")
missing_rows  <- res$bad
```
...
res <- GET("https://onemap.gov.sg/api/common/elastic/search",
           query = list(searchVal = "048940", returnGeom = "Y", getAddrDetails = "Y", pageNum = "1"))
json <- content(res)
json$found
as.data.frame(json$results) %>% dplyr::select(SEARCHVAL, POSTAL, LATITUDE, LONGITUDE, ADDRESS)
```

```{r}
#| eval: false

biz_56140_24_sf <- read_rds("data/rds/biz_56140_24_sf.rds")
```
...
```{r}
#| eval: false
all_biz_24 <- bind_rows(
  biz_46900_24_sf %>% mutate(SSIC = "Wholesale Trade (46900)"),
  biz_62011_24_sf %>% mutate(SSIC = "Software (62011)"),
  biz_56140_24_sf %>% mutate(SSIC = "Food Stalls (56140)")
)

ggplot(all_biz_24, aes(x = MONTH_ABBR)) +
  geom_bar() +
  facet_wrap(~SSIC, scales = "free_y") +
  labs(x = "Month", y = "Count")
```
...
#| eval: false
tmap_mode('view')
tm_shape(biz_46900_24_sf) +
  tm_dots(col = "red",    size = 0.5, alpha = 0.5) +
tm_shape(biz_62011_24_sf) +
  tm_dots(col = "blue",   size = 0.5, alpha = 0.5) +
tm_shape(biz_56140_24_sf) +
  tm_dots(col = "yellow",  size = 0.5, alpha = 0.5)



biz_56140_sf1 <- st_join(biz_56140_sf, mpsz_cl["PLN_AREA_N"])

biz56140_summary <- biz_56140_sf %>%
  group_by(PLN_AREA_N) %>%
  summarise(count = n()) %>%
  arrange(desc(count))
print(biz56140_summary)
```
...
biz46900_summary1 <- biz_46900_sf1 %>%
  ...
  arrange(desc(count))
print(biz46900_summary0)
print(biz46900_summary1)

```

```{r}
#| eval: false
biz_62011_sf1 <- st_join(biz_62011_sf, mpsz_cl["PLN_AREA_N"])
biz62011_summary <- biz_62011_sf1 %>% group_by(PLN_AREA_N) %>% 
  summarise(count = n()) %>% 
  arrange(desc(count))
print(biz62011_summary)
```
...
```{r}
L_46900_dc = Lest(biz46900_dc_ppp, correction = "Ripley")
plot(L_46900_dc, . -r ~ r, 
     ylab= "L(d)-r", xlab = "d(m)", 
     xlim=c(0,1000))
```
...
```{r}
set.seed(1234)
L_46900_dc.csr <- envelope(biz46900_dc_ppp, Lest, 
                     nsim = 99, 
                     rank = 1, 
                     glocal=TRUE)
```
...
Lcsr_df <- as.data.frame(L_46900_dc.csr)
...
```{r}
biz56140_month_owin <- biz56140_month_ppp[sg_owin]
...
# save to file
image_write(anim, "st_kde_biz56140_m.gif")
```
...
```{r}
biz62011_month_owin <- biz62011_month_ppp[sg_owin]
