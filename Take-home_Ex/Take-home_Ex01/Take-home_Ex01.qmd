---
title: "01: Geospatial Analytics for Public Good: Agglomeration in Play"
author: "Yiqiong PAN"
date: "Sep 10, 2025"
...
The workflow followed a two-stage SPPA and Spatial Temporal approach:

-   First-order analysis: Clark–Evans tests were applied, both under and without Complete Spatial Randomness (CSR) assumptions, to evaluate whether businesses were randomly distributed. Kernel Density Estimation (KDE) was conducted to visualise spatial concentrations.

-   Second-order analysis: L-funtion was used to analyse the internal relationship within each business type. K-fucntion was tried out for ST SPPA.

-   Subzone zoom-in: High-density subzones were identified and interpreted individually for each business type.
...
The nearby islands were **excluded** from this study due to minimal business activity. The western islands (e.g., Jurong Island, a petroleum hub), southern islands (tourism-focused, e.g., Sentosa), and north-eastern islands (military use) are outside the observation scope. One wholesale trade registration *(UEN: 202513500G)* geocoded to the Western Islands was excluded. Although the firm’s website lists its activity as “pharmaceutical solutions,” it is SSIC-coded under wholesale trade. Given the large sector base, excluding this single case does not affect results.
...
The `count()` and `slice_max()` functions from the `dplyr` package (tidyverse family) are used for data count and ranking. The code chunk below derives the top 15 new business registrations in early 2025 and adds SSIC description for easy interpretation.
...
After comparison, we decide to use `diggle` bandwidth to visualise the KDE.
...
As anticipated, similar to the pattern observed for software development, KDE at the national level for wholesale trade is dominant by central areas, due to the very high concentration of businesses there. Nevertheless, we present a high-quality KDE map at the national scale using tmap before proceeding to subzone-level analysis.
...
tm_shape(kde_biz46900_terra) + # load the data source for the layers that follow
  ...
  tm_compass() + # adds a compass
  tm_layout(scale = 1.0) + 
  tm_title(" KDE for Wholesale 46900", size = 2, position = c("left", "top"))# keeps default size
```
:::
...
The code chunk below subsets the data set to the study areas, re-scales the unit of measurement from metre to kilometre and finally plot the areas with food stalls points.
...
-   **Downtown Core** sits at the heart of Singapore, offering a prestigious blend of office towers and commercial hubs
...
Here we can see initial negative values (insignificant) under 35 m, and later the solid line shows a few significant clustering (above the grey envelope) at 135 m-160m, again at over 1100m. This pattern is probably due to the presence of large neighbourhoods and multiple hawker centres within these distances.
...
Here we can see initial negative values (insignificant) under 20 m, and later the solid line shows a few significant clustering (above the grey envelope) at 50-150m, again from 340m to 360m, indicating local and neighbourhood-scale aggregation.
:::
...
Here we can see solid line shows strong significant clustering (above the grey envelope) from 0 m to 750 m. Further new businesses are heavily clustered from 0m to 100m, probably at co-working spaces.
...
frames <- lapply(sel, function(i) {
  ...
  plot(st_kde_biz56140_m, tselect = tsel, override.par = FALSE, fix.range = TRUE,
       main = paste("56140 STKDE – MONTH", tsel))
...
anim <- image_animate(image_join(frames), delay = 80, loop = 0)
...
When using plug-in rule in spattemp.density(), the spatio-temporal bandwidths are 856m and 0.5 days.

The code chunk below applies boot.spattemp() generated optimal bandwidths 444m and 35 days to STKDE computation.

```{r}
#| eval: false
set.seed(1234)
BOOT.spattemp(biz62011_month_owin)
```

```{r}
kde_62011_boot <- spattemp.density(biz62011_month_owin,
                                  h = 444,
                                  lambda = 1.15)
summary(kde_62011_boot)
```
...
Here we use `magick` package to animate the `kde_62011_boot` file.

```{r}
# pick all 6 months
sel <- seq_along(kde_62011_boot$tgrid)

frames <- lapply(sel, function(i) {
  tsel <- kde_62011_boot$tgrid[i]
  img  <- image_graph(width = 800, height = 800, res = 96) # open a plot device

  plot(kde_62011_boot, tselect = tsel, override.par = FALSE, fix.range = TRUE,
       main = paste("62011 BOOT STKDE – MONTH", tsel))
...
# save to file
image_write(anim, "img/kde_62011_boot.gif")
```

![](img/kde_62011_boot.gif)
:::
...
When using plug-in rule in spattemp.density(), the spatio-temporal bandwidths are 919m and 0.3 days.

The code chunk below applies boot.spattemp() generated optimal bandwidths 447m and 24 days to STKDE computation.

```{r}
#| eval: false
set.seed(1234)
BOOT.spattemp(biz46900_month_owin)
```

```{r}
kde_46900_boot <- spattemp.density(biz46900_month_owin,
                                  h = 447,
                                  lambda =0.8)
summary(kde_46900_boot)
```
...
Here we use `magick` package to animate the `st_kde_biz46900_m` file.
...
Here we use `magick` package to animate the `kde_46900_boot` file.

```{r}
#| eval: false
# pick all 6 months
sel <- seq_along(kde_46900_boot$tgrid)

frames <- lapply(sel, function(i) {
  tsel <- kde_46900_boot$tgrid[i]
  img  <- image_graph(width = 800, height = 800, res = 96) # open a plot device

  plot(kde_46900_boot, tselect = tsel, override.par = FALSE, fix.range = TRUE,
       main = paste("46900 STKDE – MONTH", tsel))
...
# save to file
image_write(anim, "img/kde_46900_boot.gif")
```

![](img/kde_46900_boot.gif)
:::
...
frames <- lapply(sel, function(i) {
  ...
  img  <- image_graph(width = 800, height = 900, res = 96) # open a plot device
...
Here we test the second order ST-SPPA under `stpp` methods. The results are preliminary and further data cleaning might needed.

::: panel-tabset
### Food Stalls 56140

```{r}
coords_56140 <- st_coordinates(biz_56140_sf)
biz56140_df <- data.frame(
  x = coords_56140[, 1],  
  y = coords_56140[, 2],
  t = biz_56140_sf$`MONTH_NUM`)
biz56140_stpp <- as.3dpoints(biz56140_df)
class(biz56140_stpp)
plot(biz56140_stpp)
...
```{r}
biz56140_stik <- STIKhat(biz56140_stpp)
plotK(biz56140_stik)
```
...
```{r}
coords_62011 <- st_coordinates(biz_62011_sf)
biz62011_df <- data.frame(
  x = coords_62011[, 1],  
  y = coords_62011[, 2],
  t = biz_62011_sf$`MONTH_NUM`)
biz62011_stpp <- as.3dpoints(biz62011_df)
class(biz62011_stpp)
plot(biz62011_stpp)
...
```{r}
biz62011_stik <- STIKhat(biz62011_stpp)
plotK(biz62011_stik)
```
...
```{r}
coords_46900 <- st_coordinates(biz_46900_sf)
biz46900_df <- data.frame(
  x = coords_46900[, 1],  
  y = coords_46900[, 2],
  t = biz_46900_sf$`MONTH_NUM`)
biz46900_stpp <- as.3dpoints(biz46900_df)
class(biz46900_stpp)
plot(biz46900_stpp)
...
```{r}
biz46900_stik <- STIKhat(biz46900_stpp)
plotK(biz46900_stik)
```
:::
...
+-------------------+--------------+------------------+---------------------------------------------------------------------------+------------------------------------------+---------------------------------------------------------------------+
| Business          | C-E Test CSR | KDE              | Subzone KDE                                                               | L-Function                               | ST KDE                                                              |
+===================+==============+==================+===========================================================================+==========================================+=====================================================================+
| Food Stalls 46900 | Clustered    | Central, North   | Geylang (Aljunied East), Woodlands (E/W), Sembawang (N)                   | Geylang (35-530m),                       | Surge from May in Geylang and Punggol                               |
|                   |              |                  |                                                                           |                                          |                                                                     |
|                   |              |                  |                                                                           | Woodlands ( 135 m-160m & over 1100m),    |                                                                     |
|                   |              |                  |                                                                           |                                          |                                                                     |
|                   |              |                  |                                                                           | sembawang (50-150m & 340m-360m)          |                                                                     |
+-------------------+--------------+------------------+---------------------------------------------------------------------------+------------------------------------------+---------------------------------------------------------------------+
| Software Dev      | Clustered    | Dominant Central | Downtown Core (Cecil), Singapore River (Boat Quay), Tampines (NW hotspot) | Singapore River (0-100m up to 300m)      | Increasing Density in Jan -Feb and May - Jun in Central and Geylang |
|                   |              |                  |                                                                           |                                          |                                                                     |
| 62011             |              |                  |                                                                           | Downtown Core (strong 0-100m up to 750m) |                                                                     |
|                   |              |                  |                                                                           |                                          |                                                                     |
|                   |              |                  |                                                                           | Tempines( 50m, 150m, over 1.4km)         |                                                                     |
+-------------------+--------------+------------------+---------------------------------------------------------------------------+------------------------------------------+---------------------------------------------------------------------+
| Wholesale Trade   | Clustered    | Dominant Central | Downtown Core (Cecil), Geylang (Geylang East), Jurong East (E)            | Jurong East (0-70m up to 300m),          | Hot in Jan, May, June in Central and Geylang                        |
|                   |              |                  |                                                                           |                                          |                                                                     |
| 56140             |              |                  |                                                                           | Downtown Coil (0-50m up to 120m)         |                                                                     |
|                   |              |                  |                                                                           |                                          |                                                                     |
|                   |              |                  |                                                                           | Geylang( 0-100m up to 900m)              |                                                                     |
+-------------------+--------------+------------------+---------------------------------------------------------------------------+------------------------------------------+---------------------------------------------------------------------+

## Discussion

The selected business types, Food Stalls, Software Development, and Wholesale Trade, all exhibit agglomeration economies, but in different ways.

To strengthen the analysis, we also reviewed the 2024 new registry data. Software and wholesale businesses ranked among the top sectors in both 2024 and early 2025. In 2025, registrations became increasingly concentrated, with hundreds of new firms located in a few buildings such as City House in Downtown, Paya Lebar Square in Geylang, and 68 Circular Road in Boat Quay. Many of these buildings host co-working spaces or virtual offices, which are cost-efficient, provide easy access to services and networking opportunities, and offer postcode prestige for startups.

This pattern suggests rising demand for smaller and more flexible office spaces. Authorities may wish to encourage the growth of co-working facilities in regional centres such as Tampines or Jurong East to distribute activity more evenly. However, a cautious approach and further study are needed, as these hotspots do not necessarily reflect actual land use or employment density.

Food stalls are commonly established together in hawker centres near residential areas or public transports, where owners benefit from shared facilities and a steady stream of customer traffic. These clusters not only generate economies of scale but also bring vibrancy to neighbourhoods. Our analysis shows a surge in Geylang, which resonates with the clustering patterns observed in the other business types.
